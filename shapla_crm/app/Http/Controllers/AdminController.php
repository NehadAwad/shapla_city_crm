...
class AdminController extends Controller
{
    ...
    public function dashBoardCal(){
        ...
        ->select('total_amount')
        ->get();
        //dd($allTotalAmount->sum('total_amount'));

        //Total user due sum
        $allDueAmount = DB::table('users')
        ->select('due_amount')
        ->get();
        //dd($allDueAmount->sum('due_amount'));
    ...
    public function update_member(Request $request,$id){
        $validatedData = $request->validate([
            ...
            'mobile_1' => 'required|string|max:11',
            'mobile_2' => 'required|string|max:11',
            'birth_date' => 'required',
            'NID' => 'required',
    ...
    public function basicUpdate(Request $request,$id){
       ...
       //Date
        ...
        //update due calculation
        $add_user->due_amount = $add_user->total_amount - $add_user->booking_money_paid - $add_user->down_payment_paid - $add_user->car_parking_paid - $add_user->land_filling_1_paid - $add_user->land_filling_2_paid - $add_user->building_pilling_paid - $add_user->first_roof_casting_paid - $add_user->finishing_work - $add_user->after_handover_money;
